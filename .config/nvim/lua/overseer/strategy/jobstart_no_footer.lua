...
function JobstartNoFooterStrategy.new(opts)
    ...
    setmetatable(strategy, { __index = JobstartNoFooterStrategy })
    ---@type overseer.JobstartNoFooterStrategy
    return strategy
end

function JobstartNoFooterStrategy:reset()
...
function JobstartNoFooterStrategy:get_bufnr()
    return self.bufnr
end
...
---@param strat overseer.JobstartNoFooterStrategy
...
function JobstartNoFooterStrategy:_create_terminal()
    if
        not self.bufnr
...
function JobstartNoFooterStrategy:_init_buffer()
    ...
    if self.opts.use_terminal then
        ...
        local function open_input()
            local prompt =
                vim.api.nvim_buf_get_lines(self.bufnr, -2, -1, true)[1]
...
function JobstartNoFooterStrategy:start(task)
    ...
    local opts = vim.tbl_extend('force', wrap, {
        ...
        width = coalesce(wrap.width, vim.o.columns - 40 + 3 - 4),
...
function JobstartNoFooterStrategy:stop()
    if self.job_id and self.job_id > 0 then
        vim.fn.jobstop(self.job_id)
        self.job_id = nil
    end
...
function JobstartNoFooterStrategy:dispose()
    self:stop()
    util.soft_delete_buf(self.bufnr)
end

return JobstartNoFooterStrategy
